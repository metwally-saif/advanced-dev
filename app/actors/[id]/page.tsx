...
async function getActorById(id: string, page: number = 1, limit: number = 5) {
  return await unstable_cache(
    async () => {
      // Get actor details
      const actorData = await db.query.actor.findFirst({
      ...
      // Get all movies this actor appears in with ratings
      const moviesWithRatings = await db
        ...
        .where(and(eq(movieActors.actorId, id), eq(Movies.published, true)))
        .groupBy(
          Movies.id,
          Movies.title,
          Movies.description,
...
export async function generateMetadata({ params }: { params: { id: string } }) {
  ...
  return {
    ...
    description:
      data.actor.description || `Information about ${data.actor.name}`,
    openGraph: {
      images: [{ url: data.actor.image || "/placeholder-actor.jpg" }],
    },
  };
}
...
export default async function ActorPage({
  ...
  const actorData = await getActorById(id, page, 5);

  if (!actorData) {
    notFound();
  }

  const { actor, topMovies, allMovies } = actorData;
  const hasMedia =
    actor.media && Array.isArray(actor.media) && actor.media.length > 0;
        ...
        <div className="flex-1">
          ...
          {actor.age && (
            <p className="mb-3 text-gray-700 dark:text-gray-300">
              <span className="font-medium">Age:</span> {actor.age} years
            </p>
          )}
      ...
      {hasMedia && (
        <section className="mb-12">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {actor?.media?.map((mediaUrl, index) => (
              <div
                key={mediaUrl}
...
function Pagination({
  ...
  const getPageLink = (page: number) => {
    ...
    return `/actors/${actorId}?${params.toString()}`;
  };
